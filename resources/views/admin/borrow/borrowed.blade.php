...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">Borrow - To Return</div>
                <div class="card-body">
                    ...
                    <div class="btn-group mt-2 float-end">
                        <button class="btn btn-primary batchSelectReturn">Returned</button>
                    </div>
                </div>
            </div>
        </div>
...
<form action="{{ route('admin.borrow.borrowed.return') }}" method="post" id="batchFormReturn">
    @csrf
...
@section('internal-scripts')
    <script>
        $(document).ready(() => {
            ...
            $('.batchSelectReturn').click(function() {

                if (confirm("{{ __('messages.sure_return') }}") == true) {
                    if ($('#itemTable tr.selected').length > 0){
                        ...
                        $('#batchFormReturn input[name="ids"]').val(batchIds.join(','))
                        $('#batchFormReturn').trigger('submit')
                    }
                }
            })
        });
